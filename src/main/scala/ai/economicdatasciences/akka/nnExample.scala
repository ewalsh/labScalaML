...
import ai.economicdatasciences.akka.NNmaster
import ai.economicdatasciences.akka.NNmaster.{Prediction, QueryInput}
...
class AkkaNNExample extends Actor{
  ...
  val data = Source.fromFile("data/ionosphere.data")
    .getLines()
    .map(x => line2Data(x))
    .toList
