...
import ai.economicdatasciences.unsupervised.KMeans
...
object KMeansExample extends App {
  ...
  val srDataset = Source.fromFile("data/311_Service_Requests_for_2009.csv").getLines().map(line =>
    line.split(",")).filter(_(5) == "Noise").filter { splitLine =>
  ...
  val clusters = KMeans.cluster(data = srDataset,
    numClusters = 6,
